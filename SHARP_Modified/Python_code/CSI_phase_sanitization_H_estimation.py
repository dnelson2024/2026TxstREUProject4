...
import argparse
import time
from SHARP_Modified.Python_code.optimization_utility import *
from os import listdir, path
import pickle
from SHARP_Modified.Python_code.plots_utility import *
...
if __name__ == '__main__':
    ...
    for name in names:
        #added to make sure double work isnt done
        all_done = all(
            path.exists('./phase_processing/Tr_vector_' + name + '_stream_' + str(s) + '.txt')
            for s in range(4)
        )
        if all_done:
        ...
        for stream in range(0, 4):
            #addded to ensure that double work isnt being done
            if path.exists('./phase_processing/Tr_vector_' + name + '_stream_' + str(stream) + '.txt'):
                print('Already processed: ' + name + ' stream ' + str(stream))
                continue
            stream_start_time = time.time()
            # original code follows from here
            ...
            # added code to look at the time it takes each thingy to run
            stream_elapsed = time.time() - stream_start_time
            print('  stream %d done in %.1f s (%.1f min)' % (stream, stream_elapsed, stream_elapsed/60))

        name_elapsed = time.time() - name_start_time
        print('Finished %s in %.1f s (%.1f min)' % (name, name_elapsed, name_elapsed / 60))
